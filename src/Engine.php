...
//        $primaryKey = null;
        foreach ($searchableObjects as $object) {
            ...
            $data[$indexUid][] = $searchableArray + [/*$object->getPrimaryKey() => */'objectID' => $this->normalizeId($object->getId())];
...
//            dump(addObjects: $objects, toIdx: $indexUid);
            $result[$indexUid] = $this->client
                ->index($indexUid)
                ->addDocuments($objects, /*$primaryKey*/'objectID');
        }
dump($result);
        return $result;
    }
